...
collapses it to the package tier (plus the loose modules beside the
packages), and fails on any cycle it finds.
...
def _package_of(file_path: str, source_roots: list[str]) -> str | None:
    """The graph node a file belongs to, or None if it belongs to none.

    A file is in a package when, below its source root, it sits at least one
    directory deep inside the root package -- ``oceens/core/seed.py`` yields
    ``oceens.core``. A loose module at the root package level is a node of its
    own -- ``oceens/seed.py`` yields ``oceens.seed`` -- so that a cycle routed
    through it (``core -> seed -> services -> core``) is still a cycle. The
    root package's own ``__init__.py`` belongs to no node.
    """
    path = Path(file_path)
    for root in source_roots:
        ...
        if len(segments) == 2 and segments[1] != "__init__.py":
            return f"{segments[0]}.{Path(segments[1]).stem}"
        return None
    return None
